...
class Vehicle(RoadObject):
    ...
    """ Vehicle width [m] """
    DEFAULT_SPEEDS = [2, 14]
    """ Range for random initial speeds [m/s] defuault_speed from interaction dataset"""
    MAX_SPEED =12
    ...
    def stay_on_lane(self) ->None:
        self.stay_onL=True
        # check if the ego vehicle stay on the merge lane.
        if self.position[1]>90:
            self.stay_onL=self.lane.on_lane(self.position,margin=0.3)
        else:
            self.stay_onL=self.lane.on_lane(self.position,margin=1)
    ...
    def destination_direction(self) -> np.ndarray:
        if (self.destination != self.position).any():
            ...
            return np.zeros((2,))
    #chris change
    def to_dict(self, origin_vehicle=None, observe_intentions=True,env_vehicle=None):
