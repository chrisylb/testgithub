...
class KinematicObservation(ObservationType):
    ...
    #FEATURES: List[str] = ["distance_goal","distance_ego_vehichle", 'x', 'y', 'vx', 'vy']
    ...
    def observe(self) -> np.ndarray:
        ...
        # Add ego-vehicle
        df = pd.DataFrame(self.observer_vehicle.to_dict_s(),index=[0])
        # Add nearby traffic
        ...
        if close_vehicles:
            ...
            for v in close_vehicles[-self.vehicles_count + 1:]:
                df = df.append(v.to_dict_s(),ignore_index=True)
        # Normalize and clip
        if self.normalize:
    ...
    #    self.env.np_random.shuffle(obs[1:])
        ...
        return obs
        '''
        #chris change ,env_vehicle=self.observer_vehicle
        # Normalize and clip
        #chris change
        if self.normalize:
           df = self.normalize_obs(df)
       # df["x"]=df["x"]/100
        ...
        # Fill missing rows
        if df.shape[0] < self.vehicles_count:
            rows = np.zeros((self.vehicles_count - df.shape[0], len(self.features)),dtype='float16')
            df = df.append(pd.DataFrame(data=rows, columns=self.features), ignore_index=True)
            del rows
        # Reorder ["distance_goal","distance_ego_vehichle","vx","vy"])
        df = df[self.features]
        obs = df.values.copy()
        if self.order == "shuffled":
            self.env.np_random.shuffle(obs[1:])
        ...
        for v in self.env.road.vehicles:
            i=0
            obs[i][0]=v.position[0]
            obs[i][1]=v.position[1]
            obs[i][2]=v.velocity[0]
            obs[i][3]=v.velocity[1]
            i=i+1
